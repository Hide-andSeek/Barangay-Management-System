...
$user_no=htmlspecialchars($_POST['user_no']);
...
if(empty($_POST["username"]) || empty($_POST["user_no"]))
	{
        ...
        $sth=$db->prepare("SELECT * FROM usersdb WHERE username=:username");
        $sth->execute(array(':username'=>htmlspecialchars($_POST['username'])));
        $row=$sth->fetch(PDO::FETCH_ASSOC);
        $count=$sth->rowCount();
        if($count==1)
                {
        if (password_verify(htmlspecialchars($_POST['user_no']) , $row['user_no']))
        {
