...
if (isset($_POST['btnDelete'])) {
	...
	$sql_query = "DELETE FROM announcement_category 
									WHERE cid = ?";
	...
	// get image file from table
	$sql_query = "SELECT announcement_image 
	...
	while ($stmt_menu->fetch()) {
		$news_image = $image_data[announcement_image];
	...
	// delete data from menu table
	$sql_query = "DELETE FROM tbl_announcement 
									WHERE cat_id = ?";
...
if (isset($_POST['btnAddAnnouncement'])) {
	$announcement_heading = $_POST['announcement_heading'];
	$cid = $_POST['cid'];
	...
	if (empty($announcement_heading)) {
		$error['announcement_heading'] = " <span class='label label-danger asterisk'>Required, please fill out this field!!</span>";
	}

	if (empty($cid)) {
		$error['cid'] = " <span class='label label-danger asterisk'>Required, please fill out this field!!</span>";
	}

	if (empty($announcement_date)) {
		$error['announcement_date'] = " <span class='label label-danger asterisk'>Required, please fill out this field!!</span>";
	}

	if (empty($announcement_description)) {
		$error['announcement_description'] = " <span class='label label-danger asterisk'>Required, please fill out this field!!</span>";
	}
...
if (isset($_POST['btnDeleteAnnouncement'])) {
	...
	if ($stmt->prepare($sql_query)) {
		...
		// store result 
		$stmt->store_result();
		$stmt->bind_result($announcement_image);
		$stmt->fetch();
		$stmt->close();
	}

	// delete image file from directory
	$delete = unlink('upload/' . "$announcement_image");
	$delete = unlink('upload/thumbs/' . "$announcement_image");
